...
function eventHandler() {
  ...
  function calcRegHeaderH() {
    document.documentElement.style.setProperty('--sReg-header-h', `${regHeader.offsetHeight}px`);
  }

  if (regHeader) {
    window.addEventListener('resize', calcRegHeaderH, {passive: true});
    window.addEventListener('scroll', calcRegHeaderH, {passive: true});
    calcRegHeaderH();
  }
  ...
  function calcInpNavW() {
    document.documentElement.style.setProperty('--inp-nav-w', `${inpNav.offsetWidth + 12}px`);
  }

  if (inpNav) {
    window.addEventListener('resize', calcInpNavW, {passive: true});
    window.addEventListener('scroll', calcInpNavW, {passive: true});
    calcInpNavW();
  }
  ...
  function checkSliderOverflow(prev, next){
    $(prev).hasClass('swiper-button-disabled') ?
      $(prev).closest('.sFresh-wrap-js').addClass('before-hidden'):
      $(prev).closest('.sFresh-wrap-js').removeClass('before-hidden');

    $(next).hasClass('swiper-button-disabled') ?
      $(next).closest('.sFresh-wrap-js').addClass('after-hidden'):
      $(next).closest('.sFresh-wrap-js').removeClass('after-hidden');
  }
